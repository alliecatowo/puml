...
"#;
    ...
    let network_x =
        svg_rect_attr_before_text(&svg, "class=\"nwdiag-network\"", "public (10.0.0.x)", "x")
            .expect("network x");
    ...
    let label_x = svg_text_attr(&svg, "public (10.0.0.x)", "x").expect("label x");

    assert!(
        label_x + approx_text_width("public (10.0.0.x)", 13) <= network_x + network_width - 8,
