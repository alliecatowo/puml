...
- Decision: Add bounded include-id extraction for `!include file!TAG` using local `!startsub TAG`/`!endsub` blocks. URL includes are a surface-specific policy: the native CLI supports them for PlantUML compatibility behind `--allow-url-includes`, default runs reject them with a deterministic diagnostic, and LSP/WASM surfaces avoid surprise network access.
...
- Impact: User-facing docs and CLI help should describe language surfaces as first-class and compatibility/determinism as policy controls rather than product tiering. Do not use this decision as a blanket claim that all official PlantUML syntax has current semantic or pixel parity; current status must be verified through focused issues, tests, fixtures, and examples.
