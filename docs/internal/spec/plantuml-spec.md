...
13. **Gantt** — 29✅/39 (74%). Solid core with deterministic date-builtins, same-line start/end duration resolution, aliases, working-day offsets, `then`, resource off-days, notes, and deletion metadata covered; remaining gaps are mostly calendar-language/styling/row-placement long tail.
14. **Object** — 3✅/8 (38%).
15. **YAML** — 3✅/8 (38%).
...
26. **Sprites** — 10✅/11 (91%). Renderer and CLI sprite support have landed, including bundled OpenIconic, Bootstrap Icons, and Google Material Icons SVG label icons; only the GUI import helper is intentionally out of scope.
...
The initial audit found no general sprite system, but current code supports sprite definitions, `<$name>` references, bundled OpenIconic `<&name>` / `&name` SVG icons, bundled Bootstrap Icons `<$bi-name>` SVG sprites, bundled Google Material Icons `<$ma_name>` SVG sprites, `listsprites`, stdlib sprite includes, and `-encodesprite`. Treat chapter 23 as mostly implemented and check `audit/ch23-sprites.md` before opening new sprite work; remaining follow-up should focus on family-specific layout polish and macro-library behavior rather than rebuilding the core sprite parser.
