...
| Oracle activation | dry-run fixture category schema | `scripts/differential_oracle_smoke.py --dry-run` | Emits schema `1.1.0` metadata with no local render, no Java, no JAR, and no required build/runtime dependency | `tests/oracle_smoke.rs::differential_oracle_dry_run_schema_lists_fixture_categories` |
| Oracle activation | fixture-backed drift categories for partial PlantUML gaps | `tests/fixtures/families/valid_salt_login_form.puml` + `tests/fixtures/families/valid_chart_bar_quarterly.puml` + `tests/fixtures/errors/invalid_preproc_dynamic_invoke.puml` | Dry-run report classifies known partials as expected `drift` or `jar-only` categories until implementation closes the gap | `tests/oracle_smoke.rs::differential_oracle_dry_run_schema_lists_fixture_categories` |
