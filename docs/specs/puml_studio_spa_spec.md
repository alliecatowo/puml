...
- `compile_json(source)` and `compile_json_with_frontend(source, frontend)` return schema `puml.compile`; they are the first WASM Studio compile DTOs, returning SVG pages, family, diagnostics, `semanticTokens`, and the shared `puml.languageService` editor surface
- `language_service_json()` returns schema `puml.languageService` for worker/editor adapters that need family, completion, syntax, and semantic-token metadata without compiling a document first
