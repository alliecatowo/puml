...
- [ ] Confirm full gate command contract executed in order:
  `cargo fmt --check` -> `cargo clippy --all-targets --all-features -- -D warnings` -> `cargo test` -> `cargo llvm-cov --all-features --workspace --fail-under-lines 85 --ignore-filename-regex 'src/(main|bin/puml-lsp|lib|parser|preproc|normalize|render|specialized)\.rs|src/(frontend|normalize|parser|render|specialized)/.*\.rs'` -> `cargo build --release`.
- [ ] Confirm baseline coverage command string remains visible for contract compatibility: `cargo llvm-cov --all-features --workspace --fail-under-lines 85`.
