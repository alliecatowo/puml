...
| Chart diagrams (`@startchart`) | Supported | Parses bar/column/line/pie plus selected area/scatter forms, label/value and colon-delimited points, palette/caption/annotation hooks, axis ranges/tick-step/categories, per-axis color/text/grid suffixes, and styled legend positioning; full chart parity remains partial. |
...
| `!define` / `!undef` | Supported | Simple token substitution and function-like macro substitution before normalizer. |
...
| `!function` / `!procedure` / `!return` | Supported | User-defined functions and procedures with default/keyword/unquoted args; `$`-prefixed procedure names may be invoked directly; callable bodies use local variable state; `!return` exits functions. |
| Preprocessor builtins | Supported | `%strlen`, `%size`/`%count` (string length or JSON/list/map cardinality), `%eval`/`%eval_int`/`%eval_bool`, `%strpos`, `%substr`, `%splitstr`/`%splitstr_regex`/`%split`, `%join`, `%list`/`%array`, `%list_get`/`%array_get` with optional fallback, `%list_add`/`%list_append`/`%list_push`, `%list_set`, `%list_remove`, `%list_sort`, `%list_contains`/`%array_contains`, `%list_is_empty`/`%is_empty`, `%list_clear`, `%list_size`/`%array_size`, `%map`/`%dict`, `%map_get` with optional fallback, `%map_put`/`%map_set`, `%map_remove`/`%map_delete`, `%map_merge`, `%map_contains_key`, `%map_contains_value`, `%map_is_empty`, `%map_clear`, `%map_size`, `%get`, `%set`/`%put`, `%remove`, `%keys`/`%values` plus `%map_keys`/`%map_values`, `%trim`/`%ltrim`/`%rtrim`, `%replace`, `%startswith`/`%endswith`/`%contains`, `%intval`, `%is_number`/`%is_int`, `%str`/`%stringify`, `%quote`/`%unquote`, `%boolval`, `%true`/`%false`/`%not`, `%upper`/`%lower`, `%chr`/`%ord`, `%dec2hex`/`%hex2dec`, `%dirpath`/`%filename`/`%filenameroot`, `%get_json_attribute`/`%json_key_exists`, `%json_set`, `%json_remove`, `%json_merge`, `%json_type`, `%json_is_valid`, `%false_then_true`/`%true_then_false`, `%invoke_procedure`, `%feature`, `%variable_exists`, `%function_exists`/`%procedure_exists`, `%newline`, `%retrieve_procedure_return`. Time/env-sensitive builtins (`%date`, `%getenv`) return empty, random-sensitive builtins (`%random`, `%rand`) return `0`, and local/remote IO helpers such as `%load_json` reject with `E_PREPROC_UNSAFE_BUILTIN` for byte-stable output. |
