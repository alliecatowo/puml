    ...
    json!({
        ...
        "nodes": model.nodes.iter().map(|n| json!({
            ...
            "kind": match n.kind {
                ...
                puml::model::StateNodeKind::Terminate => "Terminate",
                puml::model::StateNodeKind::SdlReceive => "SdlReceive",
                puml::model::StateNodeKind::SdlSend => "SdlSend",
    ...
    json!({
        ...
        "chronology_events": model
            ...
            .map(|e| json!({
                ...
                "end_day": e.end_day,
                "date_precision": e.date_precision.map(|p| p.as_str()),
                "end_date_precision": e.end_date_precision.map(|p| p.as_str())
            }))
            .collect::<Vec<_>>(),
    ...
    match model {
        ...
        NormalizedDocument::Timeline(timeline) => {
            json!({
                ...
                "chronology_events": timeline.chronology_events.iter().map(|e| json!({
                    "subject": e.subject,
                    "when": e.when,
                    "end": e.end,
                    "color": e.color,
