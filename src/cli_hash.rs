...
//!
//! Uses FNV-1a 64-bit over the raw file bytes, encoded as lowercase hex or base64.
...
pub fn run_hash(args: &HashArgs) -> Result<i32, (i32, String)> {
    ...
    let digest = match args.algo {
        HashAlgoArg::Fnv => fnv1a_u64(&bytes),
    };

    let output = match args.format {
        HashFormatArg::Hex => format!("{digest:016x}"),
        HashFormatArg::Base64 => base64_encode(&digest.to_be_bytes()),
    };
...
/// Compute a stable FNV-1a 64-bit hash over `data`.
