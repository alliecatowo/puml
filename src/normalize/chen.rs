...
pub fn normalize_chen(document: Document) -> Result<ChenDocument, Diagnostic> {
    assert_eq!(document.kind, DiagramKind::Chen);
    ...
    // Assign grid positions after all entities are known.
    // Simple grid: 3 columns, rows expanding downward.
    let cols = 3usize.max(1);
    for (idx, entity) in entities.iter_mut().enumerate() {
        entity.grid_col = idx % cols;
        entity.grid_row = idx / cols;
    }
...
fn parse_attribute_line(line: &str) -> Option<ChenAttr> {
    let lower = line.to_ascii_lowercase();
    let (kind, name_raw) = if let Some(rest) = lower.strip_prefix("key ") {
        (ChenAttrKind::Key, &line[4..])
    } else if let Some(rest) = lower.strip_prefix("[key] ") {
        (ChenAttrKind::Key, &line[6..])
    } else if let Some(rest) = lower.strip_prefix("multivalued ") {
        let _ = rest;
        (ChenAttrKind::Multivalued, &line[12..])
    } else if let Some(rest) = lower.strip_prefix("derived ") {
        let _ = rest;
...
fn parse_cardinality_suffix(lhs: &str, rhs: &str) -> (String, String, String) {
    ...
    // Ignore leading `lhs` here — we only look at rhs.
    let _ = lhs;
