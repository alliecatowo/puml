...
fn render_chen_text(doc: &ChenDocument, mode: TextOutputMode) -> String {
    ...
    if !doc.entities.is_empty() {
        ...
        for entity in &doc.entities {
            let weak = if entity.is_weak { " [weak]" } else { "" };
            lines.push(format!("  {}{}", text_value(&entity.name, mode), weak));
        }
    }
    if !doc.relationships.is_empty() {
        lines.push(format!("relationships ({})", doc.relationships.len()));
        for rel in &doc.relationships {
            let identifying = if rel.is_identifying {
                " [identifying]"
            } else {
                ""
            };
            lines.push(format!("  {}{}", text_value(&rel.name, mode), identifying));
        }
    }
    finish_text(lines)
}
