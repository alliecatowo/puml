        ...
        "call_user_func" => {
            ...
            if target.is_empty() {
                return Err(Diagnostic::error_code(
                    ...
                    "%call_user_func requires a function name argument",
                ));
            }
            let callable = state.callables.get(&target).ok_or_else(|| {
                Diagnostic::error_code(
                    "E_PREPROC_CALL_UNKNOWN",
                    format!("unknown callable `{target}`"),
                )
            })?;
            if callable.kind != PreprocCallableKind::Function {
                return Err(Diagnostic::error_code(
                    "E_PREPROC_CALL_KIND",
                    format!("`{target}` is not a function"),
                ));
            }
            let forwarded = split_args(args_raw)?
                .into_iter()
                .skip(1)
                .collect::<Vec<_>>()
                .join(", ");
            Some(execute_function_call(&target, &forwarded, state, call_depth + 1)?)
        }
        "invoke_procedure" => {
            return Err(Diagnostic::error_code(
                "E_PREPROC_DYNAMIC_UNSUPPORTED",
                format!(
                    "dynamic preprocessor procedure invocation `%{}(...)` is only supported as a line directive in this deterministic subset",
                    name
                ),
            ));
        }
