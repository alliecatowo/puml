...
fn deterministic_preproc_now_seconds(state: &PreprocState) -> i64 {
    state
        .vars
        .get("PUML_NOW")
        .or_else(|| state.vars.get("PUML_NOW_EPOCH"))
        .and_then(|value| super::includes::eval_int_expr(value))
        .unwrap_or(0)
}

fn format_preprocessor_date(
    ...
    let seconds = seconds_arg
        .and_then(super::includes::eval_int_expr)
...
fn format_preprocessor_time(format_arg: Option<&str>, state: &PreprocState) -> String {
    let format = format_arg
        .filter(|value| !value.trim().is_empty())
        .unwrap_or("HH:mm:ss");
